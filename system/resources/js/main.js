...
$(function () {
    ...
    var accountsPopup = new function() {
        ...
        this.init = function() {
			...
			$('#sign_in,#sign_in_button,#edit_billing_details,#change_password,#acc_exist').click(function() {
				var email;
				var emailValue ='';
...
function initBSS(bss_id, width, height, align, valign, titleSize, titleTop, animationOpen, animationClose) {
    Cufon.replace('#' + bss_id + ' .gui-bss-template-01 h2', {fontFamily:'Myriad Pro Regular'});
    $('#' + bss_id + ' h2:first').css({
	...
	if (false != animationOpen)
	{
		...
		$('#' + bss_id + ' a.close:first').click(function() {
			$.jajax('/jajaxserver.php?app=BannerSliderSystem&action=close', {
				post:'id=' + bss_id,
				fun:$.jajaxparse
			});
			$('#' + bss_id).css(css).hide('slide', {direction:animationClose}, 'slow');
			return false;
		});
		if ($.browser.msie && $.browser.version == 7) {
		...
		$('#' + bss_id).css(css).show('slide', {direction:animationOpen}, 'slow');
	}
	return false;
}

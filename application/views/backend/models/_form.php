<form class="edt-form" method="post" accept-charset="utf-8">
	<fieldset>
		...
		<div class="field_row <?php if(form_error('brand_id')){print('validation_error');}?>">
			<div class="field_label">Marca:</div>
			<div>
				<select name="brand_id" id="brand_id" class="form_input">
					<option value="-1">Selecione...</option>
					<? if($this->router->method == 'update'){ $data_select =  $row[0]['brand_id']; }else{ $data_select = set_value('brand_id'); } data_select($brands, $data_select);?>
				</select>
			</div>
		</div>

				...
				<div id="dt_example" class="example_alt_pagination">
					<table class="table table-condensed table-striped table-hover table-bordered pull-left" id="data-table">    
						...
						<tbody>
							...
							<? if($dados){ foreach($dados as $modelos){?>
							
							<tr class="gradeX">
								<td><?=$modelos['name'];?></td>
								<td class="hidden-phone"><?=format_date($modelos['created_in']);?></td>
								<td><?=status_select($modelos['status_id']);?></td>
								<td>
									<a href="<?=site_url('admin/modelos/editar/'.$modelos['id'].'/'.$modelos['idHash']);?>" class="actions-icons">
										<img src="<?=site_url('resources/backend/img/edit-icon.png');?>" alt="editar" class="icons">
									</a>
									<a href="<?=site_url('admin/modelos/remover/'.$modelos['id'].'/'.$modelos['idHash']);?>" class="delete-row" data-original-title="Delete">

		...
		<div class="field_row <?php if(form_error('status_id')){print('validation_error');}?>">
			...
			<div>
				<select name="status_id" id="status_id" class="form_input">
					...
					<? if($this->router->method == 'update'){ $data_select =  $row[0]['status_id']; }else{ if(set_value('status_id') == '0' or set_value('status_id') == 0 ){ $data_select = 1; }else{$data_select = set_value('status_id'); }} status_select($data_select);?>
				</select>
			</div>
		</div>
	</fieldset>

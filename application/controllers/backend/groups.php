...
class Groups extends CI_Controller{
	...
	public final function __construct()
	{
		...
		$this->load->model('group_model', 'data_model');
	...
	public final function index()
	{
		...
		$data['dados'] 		=  $this->data_model->read_pag($this->limit, @$data['config']['page_now'], @$data['config']['search_args']['search_field']);		
	...
	public final function update()
	{
		...
		$data['url_title']	= $this->parameter_model->get('system_title');
		$data['scr_title']	= $this->title[$this->router->method];
		$data['row']		= $this->data_model->by(array('id' => $this->uri->segment(3), 'idHash' => $this->uri->segment(4)));
		...
		if($this->form_validation->run() == FALSE){
			...
			redirect($this->url.$this->uri->segment(2).'/'.$this->uri->segment(3).'/'.$this->uri->segment(4));
		}
	}
